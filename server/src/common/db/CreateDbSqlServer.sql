...
--ALTER DATABASE dbDiplom SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
...
CREATE TABLE tbMessage(
    ...
    CONSTRAINT ms_fk_sender FOREIGN KEY (fkIdSender) REFERENCES tbUsers(pkIdUser) ON DELETE SET NULL,
    CONSTRAINT ms_fk_receiver FOREIGN KEY (fkIdReceiver) REFERENCES tbUsers(pkIdUser) ON DELETE SET NULL
);
GO
...
GO
-- Индекс на шаблоны по курсу
CREATE INDEX idx_cert_templates_course ON tbCertificateTemplates(fkIdCourse) WHERE isDeleted = 0;
GO
...
-- ИСПРАВЛЕНО: Индекс создан ПОСЛЕ создания таблицы tbCertificate
...
--#region ===== CRUD =====
	...
	--#region ===== GROUP =====
		...
		BEGIN
			...
			BEGIN TRY
				BEGIN TRANSACTION;
				-- ИСПРАВЛЕНО: Добавлено исключение текущей группы из проверки дубликата
				IF @name IS NOT NULL AND EXISTS (SELECT 1 FROM tbGroup WHERE name = @name AND pkIdGroup != @pkIdGroup AND isDeleted = 0)
				BEGIN
					RAISERROR('Группа с названием "%s" уже существует', 16, 1, @name);
					ROLLBACK TRANSACTION;
					RETURN;
	...
	--#region ===== LESSON =====
		...
		BEGIN
			SET NOCOUNT ON;
			BEGIN TRY
				IF @title IS NOT NULL AND EXISTS ( SELECT 1 FROM tbLessons  WHERE fkIdCourse = ISNULL(@fkIdCourse, (SELECT fkIdCourse FROM tbLessons WHERE pkIdLesson = @pkIdLesson)) AND title = @title AND pkIdLesson != @pkIdLesson AND isDeleted = 0)
				BEGIN
					RAISERROR('Урок с названием "%s" уже существует в данном курсе', 16, 1, @title);
					RETURN;
				END
	...
	--#region ===== TASK =====
		...
		BEGIN
			...
			BEGIN TRY
				BEGIN TRANSACTION;
				-- ИСПРАВЛЕНО: Добавлена проверка существования внешних ключей
				IF NOT EXISTS (SELECT 1 FROM tbCourses WHERE pkIdCourse = @fkIdCourse AND isDeleted = 0)
				BEGIN
					RAISERROR('Курс %d не найден или удалён', 16, 1, @fkIdCourse);
					ROLLBACK TRANSACTION;
					RETURN;
				END
				IF @fkIdLesson IS NOT NULL AND NOT EXISTS (SELECT 1 FROM tbLessons WHERE pkIdLesson = @fkIdLesson AND isDeleted = 0)
				BEGIN
					RAISERROR('Урок %d не найден или удалён', 16, 1, @fkIdLesson);
					ROLLBACK TRANSACTION;
					RETURN;
				END
				IF @fkIdTypeTasks IS NOT NULL AND NOT EXISTS (SELECT 1 FROM tbTypeTasks WHERE pkIdTypeTask = @fkIdTypeTasks)
				BEGIN
					RAISERROR('Тип задания %d не найден', 16, 1, @fkIdTypeTasks);
					ROLLBACK TRANSACTION;
					RETURN;
				END
		...
		BEGIN
			...
			BEGIN TRY
				BEGIN TRANSACTION;
				-- ИСПРАВЛЕНО: Добавлена проверка существования внешних ключей
				IF @fkIdCourse IS NOT NULL AND NOT EXISTS (SELECT 1 FROM tbCourses WHERE pkIdCourse = @fkIdCourse AND isDeleted = 0)
				BEGIN
					RAISERROR('Курс %d не найден или удалён', 16, 1, @fkIdCourse);
					ROLLBACK TRANSACTION;
					RETURN;
				...
				BEGIN
					...
					RETURN;
				END
				DECLARE @currentCourse INT, @currentLesson INT;
				SELECT @currentCourse = fkIdCourse, @currentLesson = fkIdLesson FROM tbTasks WHERE pkIdTask = @pkIdTask;
				
				IF @fkIdLesson IS NOT NULL
				BEGIN
					IF EXISTS (SELECT 1 FROM tbTasks WHERE title = @title AND fkIdLesson = @fkIdLesson AND pkIdTask != @pkIdTask AND isDeleted = 0)
					BEGIN
						RAISERROR('Задача с названием "%s" уже существует в данном уроке', 16, 1, @title);
						ROLLBACK TRANSACTION;
						RETURN;
				...
				BEGIN
					...
					IF EXISTS (SELECT 1 FROM tbTasks WHERE title = @title AND fkIdCourse = @targetCourse AND fkIdLesson IS NULL AND pkIdTask != @pkIdTask AND isDeleted = 0)
					BEGIN
						RAISERROR('Задача с названием "%s" уже существует в данном курсе', 16, 1, @title);
						ROLLBACK TRANSACTION;
						RETURN;
	...
	--#region ===== MATERIAL =====
		...
		BEGIN
			SET NOCOUNT ON;
			BEGIN TRY
				-- Проверки вынесены за пределы транзакции для уменьшения времени блокировки
		...
		BEGIN
			...
			BEGIN TRY
				...
				DECLARE @targetCourse INT = ISNULL(@fkIdCourse, @currentCourse);
				DECLARE @targetLesson INT = @fkIdLesson;  
		...
		BEGIN
			...
			BEGIN TRY
				BEGIN TRANSACTION;
				-- ИСПРАВЛЕНО: Материалы могут иметь связанные комментарии или зависимости, если есть
	...
	--#region ===== ATTEMPT =====
		...
		BEGIN
			...
			BEGIN TRY
				BEGIN TRANSACTION;
				-- ИСПРАВЛЕНО: Опечатка "0AND" исправлена на "0 AND"
		...
		BEGIN
			...
			BEGIN TRY
				...
				BEGIN
					...
					SELECT @existingDeletedId = pkIdOption FROM tbTestOptions WHERE fkIdQuestion = @targetQuestion AND optionText = @targetText AND isDeleted = 1 AND pkIdOption != @pkIdOption;
	...
	--#region ===== CERTIFICATE =====
		...
		BEGIN
			...
			BEGIN TRY
				...
				COMMIT TRANSACTION;
				EXEC prGetCertificatesWithTemplates @pkIdCertificate = @newCertificateId;
			END TRY
			BEGIN CATCH
				IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
				DECLARE @ErrorMessage NVARCHAR(4000) = ERROR_MESSAGE();
		...
		BEGIN
			...
			IF EXISTS (
				...
				WHERE fkIdListener = @targetListener 
				  ...
				  AND isDeleted = 0
				  AND pkIdCertificate != @pkIdCertificate
			)
			BEGIN
			...
			BEGIN TRY
				...
				WHERE pkIdCertificate = @pkIdCertificate;
				-- ИСПРАВЛЕНО: Добавлена проверка @@ROWCOUNT
				IF @@ROWCOUNT = 0 
				BEGIN
					RAISERROR('Сертификат %d не найден или удалён', 16, 1, @pkIdCertificate);
					ROLLBACK TRANSACTION;
	...
	--#region ===== CERTIFICATE-TEMPL =====
		...
		BEGIN
			...
			BEGIN TRY
				...
				COMMIT TRANSACTION;
				EXEC prGetCertificateTemplates @pkIdTemplate = @newTemplateId;
			END TRY
			BEGIN CATCH
				IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
				DECLARE @ErrorMessage NVARCHAR(4000) = ERROR_MESSAGE();
...
-- ИСПРАВЛЕНО: Триггер переписан для поддержки массовых вставок
...
BEGIN
    ...
    -- Обрабатываем каждую строку из inserted отдельно
    DECLARE cur CURSOR FOR
        SELECT i.fkIdListener, i.fkIdGroup
        FROM inserted i
        WHERE i.isDeleted = 0;

    OPEN cur;
    FETCH NEXT FROM cur INTO @listenerId, @groupId;
...
BEGIN
    ...
    DECLARE cur CURSOR FOR
        SELECT i.fkIdCourse, i.title
        FROM inserted i
        WHERE i.isDeleted = 0;

    OPEN cur;
    FETCH NEXT FROM cur INTO @courseId, @taskTitle;
